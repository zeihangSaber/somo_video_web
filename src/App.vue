...
export default {
  ...
  computed: {
    maxSlide() {
      let maxSlide = Math.max(
        Math.ceil(this.members.length / this.playerNum),
        1
      );
    ...
    mineFlag() {
      ...
      if (this.speakFlag && this.speakFlag && this.slideCount === 3)
        return true;
      if (this.speakFlag || (this.speakFlag && this.slideCount === 2))
        return true;
    ...
    nowPlayerNum() {
      return Math.min(
        this.members.length - this.playerNum * (this.realCount - 1),
        this.playerNum
      );
    }
  },
